...
func yzmaGeneratorLoader(modelPath string, gpuLayers, contextSize, batchSize, lazyMode int) (Generator, error) {
	...
	if v, ok := yzmaEnvInt("NORNICDB_HEIMDALL_LAZY_MODE"); ok && v >= localllm.LazyModeOff && v <= localllm.LazyModeOn {
		opts.LazyMode = v
	}
