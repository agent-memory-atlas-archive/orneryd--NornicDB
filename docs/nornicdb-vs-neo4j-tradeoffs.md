...
| --------------------- | -------: | -------: | -------------------------: | -------- |
| Mean query latency    |  0.23 ms | 98.64 ms | **-99.8% (432.38x ratio)** | NornicDB |
...
| ---------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Read latency**             | sub-millisecond on every query class — `customer_category_distinct_orders` is 99.9% lower than Neo4j (0.25 ms vs 227 ms; 908x ratio) | —                                                                                  | Numeric-ID secondary indexes (8B per ref instead of 40-50B UUID strings), compact edge codec, in-memory dictionary lookups, MVCC head index pointing at current bodies without scan                                         |
...
NornicDB trades a 2.62x larger raw-data bucket for **99.8% lower mean query latency (432.38x ratio)**, **55% less energy per workload**, and **99.8% less WAL space (568.5x ratio)**. Total disk footprint favors NornicDB once WAL is included. The latency and energy gains come from the index architecture (numeric IDs, compact codecs, MVCC head pointers); the body-size delta is structural to LSM vs record-store and is partially closed by the property-key dictionary.
