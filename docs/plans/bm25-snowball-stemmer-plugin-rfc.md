...
2. A build-time Snowball adapter generated by NornicDB tooling.

Plugin authors can consume generated Snowball output without writing wrappers, while NornicDB core adds no Snowball or language dependency.
...
- **Plugin ID:** Stable operator-facing identifier such as `example.ukrainian`.
...
- `Type()` must return `"bm25_stemmer"`.
- `ID()` is stable across builds and unique within a process.
- `Version()` changes whenever linguistic behavior changes.
...
The narrow API deliberately excludes initialization, start, stop, and shutdown hooks. A stemmer is pure token processing and has no service lifecycle of its own.
...
The registry is populated once during startup and frozen before any search service is constructed.
...
```text
plugins/stemmers/
  example-ukrainian.so
  example-ukrainian.stemmer.json
```
...
```json
{
  ...
  "api_version": 1,
  "type": "bm25_stemmer",
  "id": "example.ukrainian",
  "version": "1.0.0",
  "library": "example-ukrainian.so",
...
The loaded symbol must implement `stemmer.Plugin`. Manifest and runtime values for type, ID, and version must match. Mismatch rejects the plugin.
...
```text
db.nornic.search.bm25.stemmer=example.ukrainian
```
...
7. Validate API version, runtime type, ID, version, and uniqueness.
...
Instead, add a packaging command:

```text
nornicdb-admin stemmer package-snowball \
  --id example.ukrainian \
  --version 1.0.0 \
  --module ./stemmer-module \
  --source ./generated/stemmer.go \
  --output ./plugins/stemmers/example-ukrainian.so
```
...
func (generatedStemmer) Type() string    { return "bm25_stemmer" }
func (generatedStemmer) ID() string      { return generatedID }
func (generatedStemmer) Version() string { return generatedVersion }
...
Plugin authors do not write this bridge. Their generated Snowball source and its runtime dependency remain inside the plugin's build boundary. NornicDB core adds no Snowball module.

The packaging command is convenience tooling, not a package manager. It does not download `.sbl` files or Go modules, select languages, or choose dependency versions. The author supplies all source and a vendored module. Snowball runtime code remains a plugin-owned build dependency and never enters NornicDB's root `go.mod` or server binary.
...
### Snowball Packaging Tests

The packaging command receives golden tests using a small generated-style fixture with the official `Stem(*Env) bool` shape. Tests verify generated bridge source, manifest fields, digest calculation, and actionable compiler errors.
...
```text
BM25 analyzer: tokenizer=unicode-nfkc-casefold-v1 stemmer=example.ukrainian version=1.0.0 digest=abc123...
```
...
- Add `nornicdb-admin stemmer package-snowball`.
...
2. Should `nornicdb-admin stemmer package-snowball` accept only a local module with a committed `go.sum`, or also accept a single generated source file plus an explicitly pinned runtime module?
